...
def generate_scenes():
    ...
    # 1. Check for Generated Templates (Exact or Regex Match)
    
    # Geometry: "5-gon", "polygon 5", "triangle" (3), "square" (4), "pentagon" (5), "hexagon" (6)
    ...
    # Atoms: "atom 6", "element 1", "carbon" (6), "hydrogen" (1)
    ...
    # Graphs: "graph x^2", "plot sin(x)"
    # Simple mapping for common functions
    func_map = {
        "sin": "sin(x)", "cos": "cos(x)", "tan": "tan(x)", "quadratic": "x^2", "cubic": "x^3",
        "linear": "x", "exponential": "exp(x)", "log": "log(x)", "sigmoid": "sigmoid"
    }
    for k, v in func_map.items():
        if k in description and ('graph' in description or 'plot' in description):
            key = f"graph_{v}"
            if key in TEMPLATES: return jsonify(TEMPLATES[key])

    # Formulas - Enhanced Search
    # Check if any formula name exists in the description
    def normalize(s): 
    ...
    for key in TEMPLATES:
        if key.startswith('formula_'):
            # Convert key 'formula_area_of_circle' -> 'area of circle'
            raw_name = key.replace('formula_', '').replace('_', ' ')
            norm_name = normalize(raw_name)
            
            # Check if normalized name is in normalized description
            # e.g. 'bernoullisprinciple' in 'showmebernoullisprinciple'
            if norm_name in norm_desc:
                return jsonify(TEMPLATES[key])
                
            # Fallback for specific partials if needed
            if 'bernoulli' in description and 'bernoulli' in raw_name:
                 return jsonify(TEMPLATES[key])

    if 'formula' in description:
        # Try to find a formula key (redundant but kept for safety)
        for key in TEMPLATES:
            if key.startswith('formula_') and key.replace('formula_', '').replace('_', ' ') in description:
                return jsonify(TEMPLATES[key])
                
    # Vectors
    if 'vector' in description and 'random' in description:
        import random
        key = f"vector_gen_{random.randint(0, 49)}"
        return jsonify(TEMPLATES[key])

    # Derivations
    if 'derivation' in description and 'power' in description:
        import random
        key = f"deriv_x{random.randint(2, 51)}"
        return jsonify(TEMPLATES[key])

    # Algorithms
    if 'sort' in description and 'random' in description:
        import random
        key = f"sort_random_{random.randint(0, 49)}"
        return jsonify(TEMPLATES[key])

    # Matrix Operations - Dynamic
    # Matches "4x4 matrix", "3*3 matrix", "matrix 4x4", etc.
    matrix_match = re.search(r'(\d+)[\*x](\d+)', description)
    if matrix_match and ('matrix' in description or 'multiplication' in description or 'exponentiation' in description):
        rows = int(matrix_match.group(1))
        cols = int(matrix_match.group(2))
        
        # Cap size to avoid UI overflow
        rows = min(rows, 6)
        cols = min(cols, 6)
        
        op = "mult"
        if "exponentiation" in description or "power" in description:
            op = "exp"
        elif "add" in description or "sum" in description:
            op = "add"
    ...
    adv_topics = ["Determinant", "Gradient Descent", "Fourier Transform", "Riemann Sum", "Taylor Series"]
    ...
    # Advanced Template Fallbacks (for loose matching)
    advanced_map = {
        "star": "life_cycle_of_star",
        "life cycle": "life_cycle_of_star",
    ...
    for keyword, partial_key in advanced_map.items():
        if keyword in description:
            # Look for matching formula key
            for key in TEMPLATES:
                if key.startswith("formula_") and partial_key in key:
                     return jsonify(TEMPLATES[key])

    # Existing Logic Fallback
    if 'selection' in description:
        return jsonify(TEMPLATE_SELECTION_SORT)
    elif 'bubble' in description or 'sort' in description:
        if 'merge' in description:
            return jsonify(TEMPLATE_MERGE_SORT)
        return jsonify(TEMPLATE_BUBBLE_SORT)
    elif 'sine' in description or 'wave' in description:
        if 'fourier' in description:
            return jsonify(TEMPLATE_FOURIER)
        return jsonify(TEMPLATE_SINE_WAVE)
    elif 'pythagoras' in description: 
        return jsonify(TEMPLATE_PYTHAGORAS)
    elif 'vector' in description or 'add' in description or 'cross' in description or 'dot' in description:
        if 'cross' in description:
            return jsonify(TEMPLATE_CROSS_PRODUCT)
        elif 'dot' in description or 'projection' in description:
            return jsonify(TEMPLATE_DOT_PRODUCT)
        elif 'sub' in description or 'minus' in description:
            return jsonify(TEMPLATE_VEC_SUB)
        return jsonify(TEMPLATE_VECTOR_ADD)
    elif 'sphere' in description or 'circle' in description and 'geometry' not in description:
        return jsonify(TEMPLATE_SPHERE)
    elif 'matrix' in description or 'multiplication' in description:
        return jsonify(TEMPLATE_MATRIX)
    elif any(x in description for x in ['geometry', 'shape', 'polygon']):
        return jsonify(TEMPLATE_GEOMETRY_SHAPES)
    elif 'derivation' in description or 'formula' in description or 'math' in description or 'limit' in description or 'integral' in description or 'chain' in description or 'calculus' in description:
        if 'quadratic' in description or 'roots' in description:
            return jsonify(TEMPLATE_QUADRATIC)
        elif 'algebra' in description or 'expansion' in description:
            return jsonify(TEMPLATE_ALGEBRA)
        elif 'limit' in description or 'calculus' in description:
            return jsonify(TEMPLATE_LIMIT)
        elif 'integral' in description or 'area' in description:
            return jsonify(TEMPLATE_INTEGRAL)
        elif 'chain' in description or 'rule' in description:
            return jsonify(TEMPLATE_CHAIN_RULE)
        return jsonify(TEMPLATE_DERIVATION)
    elif 'graph' in description or 'plot' in description or 'chart' in description or 'bar' in description or 'race' in description and 'bfs' not in description:
        if 'pie' in description:
            return jsonify(TEMPLATE_PIE_CHART)
        elif 'race' in description or 'bar' in description:
            return jsonify(TEMPLATE_BAR_RACE)
        elif 'scatter' in description or 'regression' in description:
            return jsonify(TEMPLATE_SCATTER)
        return jsonify(TEMPLATE_GRAPH_PLOT)
    elif 'binary' in description or 'search' in description and 'bfs' not in description:
        return jsonify(TEMPLATE_ALGO_BINARY)
    elif 'solar' in description or 'planet' in description or 'system' in description:
        return jsonify(TEMPLATE_SOLAR_SYSTEM)
    elif 'dna' in description or 'gene' in description or 'helix' in description:
        return jsonify(TEMPLATE_DNA)
    elif 'mitosis' in description or 'cell' in description:
        ...
        elif 'water' in description or 'cycle' in description:
            return jsonify(TEMPLATE_WATER_CYCLE)
        return jsonify(TEMPLATE_ATOM)
    elif 'bfs' in description or 'breadth' in description or 'pathfinding' in description or 'traversal' in description:
        return jsonify(TEMPLATE_ALGO_BFS)
    elif 'stack' in description or 'push' in description or 'pop' in description:
        return jsonify(TEMPLATE_STACK)
    elif 'queue' in description or 'enqueue' in description or 'dequeue' in description:
        return jsonify(TEMPLATE_QUEUE)
    
    # Error / Fallback
    return jsonify({
        "error": "Could not parse description",
        "suggestedPrompt": "Try: 'Bubble sort', 'Solar system', 'DNA Helix', 'Graph plot', 'Binary search', 'Atom model', 'BFS Graph', 'Quadratic roots', 'Dot product', 'Pie chart', 'Algebra expansion', 'Stack operations', 'Photosynthesis', 'Calculus Limit', 'Cross Product', 'Bar Chart Race', 'Integral Area', 'Queue Operations', 'Water Cycle', 'Fourier Series', 'Vector Subtraction', 'Scatter Plot', 'Chain Rule', 'Merge Sort', 'Mitosis', 'Polygon 12', 'Atom 6', 'Graph sin(x)', 'Random Vector', 'Derivation Power Rule'",
