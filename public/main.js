...
function init() {
    ...
    if (levelParam && levelSelect.querySelector(`option[value="${levelParam}"]`)) {
...
async function handleGenerate(isRefresh = false) {
    ...
    if (!isRefresh) showLoader(true);
    try {
        const res = await fetch(`${API_URL}/generate/scenes`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ description: text, options: options })
        });
        ...
        // Save to Last Viewed for Dashboard
        try {
            const lastViewed = {
                name: text,
...
function loadScene(newScene) {
    ...
    // Update UI
    jsonEditor.value = JSON.stringify(scene, null, 2);
    setCode(scene.code || "# No code available for this scene.");
...
function checkCodeHighlight() {
    // Reset highlights
    document.querySelectorAll('.highlight-line').forEach(el => el.classList.remove('highlight-line'));

    let activeLine = -1;
    // Find the latest active action that has a codeLine
    const activeAction = [...scene.actions]
        .sort((a, b) => b.start - a.start) // Sort descending
        .find(a => currentTime >= a.start && currentTime < a.end + 0.5 && a.codeLine);

    if (activeAction) {
        highlightLine(activeAction.codeLine);
    }
}
